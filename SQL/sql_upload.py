...
def upload_job(company, raw_data) :
	...
	sql = '''INSERT INTO zd_new_job(Title, Url, Url_status, Created_on, Expired_on, Org_id, Loc_id, Snippet, tags) SELECT \'{0[0]}\', \'{0[1]}\', 200, \'{0[5]}\', \'{0[6]}\', org1.ID, loc1.ID, \'{0[7]}\', \'{0[8]}\' FROM zd_new_organization AS org1, zd_new_location AS loc1 WHERE org1.Name = \'{1}\' AND loc1.City = \'{0[2]}\' AND loc1.Abbreviation = \'{0[3]}\' AND loc1.Country = \'{0[4]}\' ON DUPLICATE KEY UPDATE Snippet = \'{0[7]}\', tags = \'{0[8]}\';'''.format(job, company)
	f.write(sql + '\n')
	# print sql
...
def update_job(company, raw_data) :
	...
	job = prepare_data(raw_data)
	sql = '''UPDATE zd_new_job AS job JOIN zd_new_organization AS org JOIN zd_new_location AS loc ON job.Org_id = org.ID AND job.Loc_id = loc.ID SET Snippet = \'{0[7]}\', tags = \'{0[8]}\', Expired_on = \'{0[7]}\' WHERE Title = \'{0[0]}\' AND job.Url = \'{0[1]}\' AND City = \'{0[2]}\' AND Abbreviation = \'{0[3]}\' AND Country = \'{0[4]}\' AND org.Name = \'{1}\';'''.format(job, company)
	f.write(sql + '\n')
